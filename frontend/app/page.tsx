...
function buildApiUrl(base: string, path: string) {
  ...
  return `${normalizePrefix(base)}${normalizedPath}`;
}
...
function parseMappings(raw: string) {
  ...
  return raw
    ...
    .filter(Boolean)
    .map((entry) => {
      const [prefix, target] = entry.split('|').map((value) => value?.trim());
      return { prefix, target };
    })
    .filter((value): value is { prefix: string; target: string } => Boolean(value.prefix && value.target));
}
...
function playerNameById(players: PlayerView[], playerId?: string | null) {
  ...
  return players.find((player) => player.id === playerId)?.username || null;
}
...
export default function Page() {
  ...
  const pushNotice = useCallback((message: string) => {
    ...
    setNotices((current) => [message, ...current].slice(0, 4));
  }, []);
    ...
    const initDiscord = async () => {
      try {
        ...
        if (!DISCORD_CLIENT_ID) {
          throw new Error('NEXT_PUBLIC_DISCORD_CLIENT_ID is missing.');
        }
        ...
        if (!tokenPayload.access_token) {
          throw new Error('Backend token endpoint did not return access_token.');
        }
        ...
        if (!auth?.user) {
          throw new Error('Discord SDK authenticate returned no user payload.');
        }
        ...
        try {
          ...
          // Non-fatal: fallback to authenticate() user payload.
        }
  ...
  useEffect(() => {
    if (authState !== 'authenticated' || !profile || !roomId) {
      return;
    }
    ...
    const socket = isAbsoluteHttpUrl(base)
      ...
      : io({
          path: `${normalizePrefix(base)}/socket.io`,
          transports: ['websocket'],
        });
    ...
    socket.on('room_state', (payload: RoomState) => {
      setRoomState(payload);
    });

    socket.on('system_notice', (payload: { message?: string }) => {
      if (payload?.message) {
        pushNotice(payload.message);
      }
    });
  ...
  const aliveTargets = useMemo(
    () => players.filter((player) => player.isAlive && !player.isHost),
    [players],
  );

  const isSleepingCitizen =
    roomState?.phase === 'NIGHT_PHASE' && selfRole === 'المواطن' && Boolean(self?.isAlive);

  const mafiaLiveSelections = useMemo(() => {
    if (!roomState?.night.mafiaSelections) {
      return [];
    }
  ...
  const resolutionText = useMemo(() => {
    ...
    if (result.type === 'NIGHT_RESULT') {
      if (result.killedPlayerId) {
        return `سقط في الليل: ${playerNameById(players, result.killedPlayerId) || 'لاعب'}`;
      }

      if (result.savedPlayerId) {
        return `تم إنقاذ: ${playerNameById(players, result.savedPlayerId) || 'لاعب'}`;
      }
    ...
    if (result.type === 'VOTE_RESULT') {
      if (result.eliminatedPlayerId) {
        return `تم إقصاء: ${playerNameById(players, result.eliminatedPlayerId) || 'لاعب'}`;
      }
      ...
      <div className="mx-auto w-full max-w-7xl">
        <header className="deco-shell rounded-2xl p-4 md:p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.4em] text-[#B9B3A7]">Dark Deco Mafia</p>
              <h1 className="font-display text-3xl md:text-4xl">مافيا Discord</h1>
                  ...
                  <div key={player.id} className={`role-card rounded-xl p-3 ${player.isAlive ? 'alive' : ''}`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-[#B9B3A7]">
                          {player.isHost ? 'الراوي / Admin' : player.isAlive ? 'حي' : 'خارج اللعبة'}
                        </p>
          ...
          <aside className="deco-shell rounded-2xl p-4 md:p-6">
            ...
            <div className="mt-4 space-y-2">
              {roomState?.phase === 'LOBBY' ? (
                <>
                  {!self?.isHost ? (
                    <button className="action-btn rounded-lg" onClick={() => emitEvent('set_ready', { ready: true })}>
                      جاهز
                    </button>
                  ) : null}
              ...
              {roomState?.phase === 'NIGHT_PHASE' && self?.isAlive ? (
                <>
                  {selfRole === 'المافيا' ? (
                    <>
                      <p className="text-xs text-[#B9B3A7]">
                        اختر هدف القتل. سيرى بقية المافيا اختيارك فوراً.
                      </p>
                      {aliveTargets
                        ...
                        <div className="mt-3 rounded-lg border border-white/10 bg-black/30 p-3">
                          ...
                          <div className="mt-2 space-y-1 text-sm">
                            {mafiaLiveSelections.map((entry) => (
                              <p key={`${entry.mafiaName}-${entry.targetName}`}>
                                {entry.mafiaName} ⟶ {entry.targetName}
                              </p>
                            ))}
                          </div>
                        </div>
              ...
              {roomState?.phase === 'DAY_PHASE' ? (
                <>
                  <div className="rounded-lg border border-white/15 bg-black/30 p-3 text-sm">
                    مرحلة النقاش النهاري فعّالة. ناقشوا ثم افتحوا التصويت.
                  </div>
                  {canHostControl ? (
                    ...
                    .map((player) => {
                      ...
                      return (
                        <button
                          key={`vote-${player.id}`}
                          className="action-btn rounded-lg"
                          onClick={() => emitEvent('cast_vote', { targetId: player.id })}
                        >
              ...
              {roomState?.phase === 'RESOLUTION' ? (
                <>
                  <div className="rounded-lg border border-white/15 bg-black/30 p-3 text-sm">
                    <p>{resolutionText}</p>
                    {roomState.winner ? (
                      <p className="mt-2 font-semibold text-[#F5D13B]">{WINNER_LABELS[roomState.winner]}</p>
                    ) : null}
                  </div>
